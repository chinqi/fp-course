...
string ::
  ...
  Parser Chars
string Nil = pure Nil
string (c :. cs) = lift2 (:.) (satisfy (== c)) (string cs)
...
hex =
  ( \h -> case readHex h of
      Empty -> P (\_ -> UnexpectedString h)
      Full x -> pure (chr x)
  )
    =<< thisMany 4 (satisfy isHexDigit)
...
betweenSepbyComma ::
  ...
  Parser a ->
  Parser (List a)
betweenSepbyComma o c p = between (is o) (is c) (sepby p (is ',' >> spaces))
