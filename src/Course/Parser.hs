...
phoneBodyParser ::
  Parser Chars
phoneBodyParser = lift2 (:.) digit (list (digit ||| is '.' ||| is '-')) ||| valueParser ""
...
phoneParser ::
  Parser Chars
phoneParser = phoneBodyParser <* satisfy (== '#')
...
personParser =
  ageParser >>=~ \age ->
    firstNameParser >>=~ \fir ->
      surnameParser >>=~ \sur ->
        smokerParser >>=~ \smo ->
          Person age fir sur smo <$> phoneParser
