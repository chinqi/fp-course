...
instance (Functor k) => Functor (StateT s k) where
  (<$>) ::
    ...
    StateT s k a ->
    StateT s k b
  (<$>) f (StateT k) = StateT (\s -> (\(a, b) -> (f a, b)) <$> k s)
...
runState' ::
  ...
  s ->
  (a, s)
runState' (StateT k) s = runExactlyOne $ k s
...
exec' ::
  ...
  s ->
  s
exec' (StateT g) s = snd $ runExactlyOne (g s)
...
eval' ::
  ...
  s ->
  a
eval' (StateT g) s = fst $ runExactlyOne (g s)
...
distinct' ::
  ...
  List a ->
  List a
distinct' xs = eval' (filtering (\a -> state' (\s -> (S.member a s, S.insert a s))) xs) S.empty
...
distinctF ::
  ...
  List a ->
  Optional (List a)
distinctF xs = evalT (filtering (StateT . distinct'') xs) S.empty
  where
    distinct'' a s = if a > 100 then Empty else Full (S.member a s, S.insert a s)
...
instance Applicative (Logger l) where
  ...
  (<*>) ::
    ...
    Logger l a ->
    Logger l b
  (<*>) (Logger x f) (Logger xs a) = Logger (x ++ xs) (f a)
...
instance Monad (Logger l) where
  (=<<) ::
    ...
    Logger l a ->
    Logger l b
  (=<<) f (Logger x a) = let (Logger xs b) = f a in Logger (x ++ xs) b
...
distinctG xs =
  runOptionalT
    (evalT (filtering (\a -> StateT (\s -> OptionalT (f s a))) xs) S.empty)
  where
    f s a
      | a > 100 = log1 ("aborting > 100: " ++ show' a) Empty
      | even a = log1 ("even number: " ++ show' a) (Full (S.member a s, S.insert a s))
      | otherwise = pure (Full (S.member a s, S.insert a s))
